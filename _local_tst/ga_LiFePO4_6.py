    ...
    for i in range(n_chidren):
        ...
        for pi, mvars, sign in zip(params, mvar, signs):
            if rnd.randint(0, 100) / 100. <= mrate:
                step = (2 * mvars[1]) / 30
                # print('xxxx',mvars[0] - mvars[1], mvars[0] + mvars[1] + step, step)
...
def mate(params, ngen,mvar, signs):
    ...
    for i in range(ns):
        ...
        param1 = mutate(params[0][:cutsite]+params[1][cutsite:], 1, 0.9, mvar, signs)[0]
...
if __name__ == '__main__':
    ...
    E0, V0, K0, K0p = -6.74512999E+05, 6.7748773153037334e-06, 154417204156.02866, 3.8444316165321997  # 6.405559904e-06, 155528389200.0, 3.931401
    nu = 0.37474100116372744  # 0.3785070135864595
    a0, m0 = 0.001717577024000639, -4.591467509788307  # 0.0017137103573339513, -12.324800843121565#0.0023833244267004285, 9.969819526649578#0.00017496414802500057, -1.463874632286355
    s0, s1, s2 = 0.014335060923440425, -207.18666861074996, -4604422.942757998  # 0.012801727590107302, -0.5200019440899462, -4551089.60942477#-3.969303090100795e-05, 2034.8571365979824, 10772545.326296955#0.009715033969061806, 8756.571746579899, -1435200717.6
    edef, sdef = 18.00000000000074, 1.6766666666666983  # 18.400000000000063, 1.7699999999999994#23.27956606428067, 1.6981192643056922#20, 2
    ...
    T = np.array([0.1,71.15,147.5362319,211.78,268.4057971,324.4,388.9855072,443.4,509.8550725,572.3,630.4347826,751.0144927])
    C_exp = np.array([0.001,4.465714286,10.16393443,14.32857143,15.38173302,18.52857143,18.68852459,21.82857143,20.83372365,24.04285714,23.35362998,23.96252927])
    ...
    gs = fig.add_gridspec(ncols=2,nrows=5, hspace=0, right=0.99, top=.99, bottom=.05, left=0.12)
    ...
    ax[0][0].plot(T_exp, Cp_exp, label = 'BATT [140]', marker='s', linestyle='None', markerfacecolor='None', markeredgecolor='purple')
    ax[0][0].plot(T_exp22, Cp_exp22/7, label = 'Loos [xx]', marker='o', linestyle='None', markerfacecolor='None', markeredgecolor='mediumpurple')
    ...
    #ax[1].plot(T_exp,err)
    ax[1][0].set_title(r'$F^{XS}=A(V)\cdot T$', y=0.7)
    ax[1][0].plot(T_JJ,Sa_JJ, label='Murnaghan', color='orchid')
    ax[1][0].plot(T_JJ,Sa_JJ_fitted, label=r'fitted $C_P$', color='C3')
    ...
    ax[0][1].plot(T_JJ, a_JJ, label='Murnaghan', color='orchid')
    ax[0][1].plot(T_JJ, a_JJ_fitted-min(a_JJ_fitted), label=r'fitted $C_P$', color='C3')
    ax[0][1].legend(loc='lower right')

    ax[1][1].set_title(r'$K_T(T)$', y=0.7)
    ax[1][1].plot(T_JJ, Kt_JJ, label='Murnaghan', color='orchid')
    ax[1][1].plot(T_JJ, Kt_JJ_fitted, label=r'fitted $C_P$', color='C3')
    ax[1][1].legend(loc='lower right')

    ax[2][1].set_title(r'$G(T)$', y=0.8)
    ax[2][1].plot(T_JJ, G_JJ, label='Murnaghan', color='orchid')
    ax[2][1].plot(T_JJ, G_JJ_fitted, label=r'fitted $C_P$', color='C3')
    ax[2][1].legend(loc='lower right')

    ax[3][1].plot(T_JJ, [a_JJi*V_JJi for a_JJi, V_JJi in zip(a_JJ,V_JJ)], label=r'$V\cdot \alpha$, Murnaghan', marker = 's', linestyle = 'None', markerfacecolor = 'None', markeredgecolor = 'orchid')
    ax[3][1].plot(T_JJ, -dSdP_T_JJ, label=r'$-\left(\frac{\partial S}{\partial P}\right)_T$, Murnaghan', marker = 'x', linestyle = 'None', markerfacecolor = 'None', markeredgecolor = 'orchid')
    ax[3][1].plot(T_JJ, [(a_JJi-a_JJ_fitted[0])*V_JJi for a_JJi, V_JJi in zip(a_JJ_fitted,V_JJ_fitted)], label=r'$V\cdot \alpha$, fitted $C_P$', marker = 's', linestyle = 'None', markerfacecolor = 'None', markeredgecolor = 'C3')
    ax[3][1].plot(T_JJ, -(dSdP_T_JJ_fitted-dSdP_T_JJ_fitted[0]), label=r'$-\left(\frac{\partial S}{\partial P}\right)_T$, fitted $C_P$', marker = 'x', linestyle = 'None', markerfacecolor = 'None', markeredgecolor = 'C3')
    ax[3][1].legend(loc='lower right')

    ax[4][1].plot(T_JJ, [dBti/Bti**2 for dBti, Bti in zip(dKtdT_P_JJ, Kt_JJ)], label=r'$\frac{1}{K_T^2}\left(\frac{\partial K_T}{\partial T}\right)_P$, Murnaghan',marker='s', linestyle='None', markerfacecolor='None', markeredgecolor='orchid')
    ax[4][1].plot(T_JJ, dadP_T_JJ, label=r'$-\left(\frac{\partial \alpha}{\partial P}\right)_T$, Murnaghan', marker='x',linestyle='None', markerfacecolor='None', markeredgecolor='orchid')
    ax[4][1].plot(T_JJ, [dBti/Bti**2 for dBti, Bti in zip(dKtdT_P_JJ_fitted, Kt_JJ_fitted)], label=r'$\frac{1}{K_T^2}\left(\frac{\partial K_T}{\partial T}\right)_P$, fitted $C_P$',marker='s', linestyle='None', markerfacecolor='None', markeredgecolor='C3')
    ax[4][1].plot(T_JJ, dadP_T_JJ_fitted, label=r'$-\left(\frac{\partial \alpha}{\partial P}\right)_T$, fitted $C_P$', marker='x',linestyle='None', markerfacecolor='None', markeredgecolor='C3')
    ax[4][1].legend(loc='lower right')

    ax[0][0].set_ylabel('$[J/K\cdot mol-at]$')
    ax[1][0].set_ylabel('$[J/K\cdot mol-at]$')
