...
class EAM_tst:  # Morse
    ...
    def E0(self, V):
        ...
        # phi_arr = list(np.zeros(len(pEOS_pt)))# np.array([[],[],[]])
        rho_arr = list(np.zeros(len(pEOS_pt)))
        factor_r = (V / self.Vstar) ** (1 / 3)

        r = self.ndist * factor_r
        phi_arr_funct = lambda alpha, beta, r_alpha: -alpha * (1 + beta * (r / r_alpha - 1)) * np.exp(-beta * (r / r_alpha - 1))
        phi_arr = np.array([phi_arr_funct(pEOS_pt[:,0],pEOS_pt[:,1],pEOS_pt[:,2])])
        ...
        F_is = np.zeros(len(self.types_new))
        for F_isix, i, rho_i in zip(range(len(F_is)), self.types_new, self.rho_is):
            F0, F1, rho_e, n = self.pEOS_et[:, int(i)]
            # ##print('F0, F1, rho_e, n',F0, F1, rho_e, n)
            # rho_i_rho_e = rho_i / rho_e
            # rho_i_rho_e_n = (rho_i / rho_e) ** n
            F_is[F_isix] = -F0 * (1 - np.log((rho_i / rho_e) ** n)) * (rho_i / rho_e) ** n + F1 * (rho_i / rho_e)#self.F_i(rho_i, F0, F1, rho_e, n)
        self.F_is = F_is
        self.Fs = np.sum(F_is)
        self.Phis = np.sum(self.ab(phi_arr, self.npair)) * self.nats / 2
